...
const int ledPin = 0;           // Pin for LED to indicate OCCUPIED - 13 on most Arduino, 0 on ATTiny
const int RelayC = 11;          // Pin for relay output when crossing gate is running
const int ledL = 7;             // Pin for one of the flasher LEDs - PWM
const int ledR = 8;            // Pin for one of the flasher LEDs - PWM

const long interval = 1000;     // On timer for the LED while flashing 
int fadeValue = 0;
int fadeValue2 = 0;
...
{                                 // Turn off RelayC
  ...
  RUNNING = LOW;
  digitalWrite(ledL, LOW);       // Make sure crossing LEDs are off
